...
import { updateRecipe } from "@/app/helpers/actions";
...
interface PrefilledValues {
  ...
  prefilledPrepTime: string;
  prefilledCookTime: string;
  prefilledServings: string;
}
...
export default function UpdateRecipe({
  ...
  const [addedIngredients, setAddedIngredients] = useState<string[]>(
    JSON.parse(prefilledIngredients)
  );
  ...
  const [addedInstructions, setAddedInstructions] = useState<string[]>(
    JSON.parse(prefilledInstructions)
  );
  const [addedInstructionsError, setAddedInstructionsError] =
    useState<boolean>(false);
  const [summary, setSummary] = useState<string>(prefilledSummary);
  const [summaryError, setSummaryError] = useState<boolean>(false);
  const [prepTime, setPrepTime] = useState<string>(prefilledPrepTime);
  const [prepTimeError, setPrepTimeError] = useState<boolean>(false);
  const [cookTime, setCookTime] = useState<string>(prefilledCookTime);
  const [cookTimeError, setCookTimeError] = useState<boolean>(false);
  const [servings, setServings] = useState<string>(prefilledServings);
